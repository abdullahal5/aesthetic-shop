...
// SIMPLIFIED: Just clean the number, no aggressive formatting
const cleanPhoneNumber = (value: string): string => {
  // Remove all non-digits
  return value.replace(/\D/g, "");
};

// Format for display only (doesn't restrict typing)
const formatForDisplay = (value: string): string => {
  const digits = cleanPhoneNumber(value);

  if (digits.length === 0) return "";

  // Format as +880XXXXXXXXX when enough digits
  if (digits.length >= 11) {
  ...
  // Show as is while typing
  return digits;
};

export default function CheckoutPage() {
  const { items, total, clearCart, appliedCoupon, discountAmount, grandTotal } =
    useCart();
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store the formatted version for display, but keep digits for validation
    set("phone", formatted);
  };
  ...
  const validate = () => {
    ...
    if (!form.phone.trim()) {
      ...
      // Get just the digits from the phone field
      const digits = cleanPhoneNumber(form.phone);

      // Check if it's a valid Bangladesh number
      // Accept 11 digits starting with 01 or 8801
      if (digits.length === 11 && digits.startsWith("01")) {
        // Valid - 01XXXXXXXXX format
      } else if (digits.length === 13 && digits.startsWith("8801")) {
        // Valid - 8801XXXXXXXXX format
      } else if (digits.length === 10 && digits.startsWith("1")) {
        // Valid - 1XXXXXXXXX format (will be converted)
      } else {
        e.phone =
          "Enter a valid 11-digit Bangladesh phone number (e.g., 017XXXXXXXX)";
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    // Clean phone number to store only digits in 8801XXXXXXXXX format
    let cleanPhone = cleanPhoneNumber(form.phone);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-3 space-y-8">
            ...
            <Field label="Phone Number" error={errors.phone} required>
              <div className="relative">
                <input
                  ...
                  className={cn(inputCls(!!errors.phone), "pl-14")}
                  maxLength={18} // Allow space for formatting
                />
              ...
              <p className="text-xs text-stone-400 mt-1">
                Enter your 11-digit Bangladesh number (e.g., 01712345678)
              </p>
            </Field>
          ...
          <section className="bg-white rounded-2xl border border-stone-100 p-6 space-y-4">
            ...
            <p className="text-xs text-stone-400 flex items-start gap-1.5">
              ...
              We&apos;ll call you at{" "}
              <strong>{form.phone || "your number"}</strong> to confirm before
              dispatch.
            </p>
          </section>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5 sticky top-24 space-y-4">
            ...
            {/* Show applied coupon in checkout */}
            {appliedCoupon && (
              <div
